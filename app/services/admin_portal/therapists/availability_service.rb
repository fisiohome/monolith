  ...
  module Therapists
    ...
    class AvailabilityService
      ...
      def available_time_slots_for_date
        ...
        # 2. Use preloaded appointments if available, otherwise fetch from database
        appointments = if @therapist.respond_to?(:preloaded_appointments)
          @therapist.preloaded_appointments || []
        else
      ...
      #
      # Fetch adjacent appointment addresses for location-based scheduling
      # Optimized to use preloaded appointments when available
      def fetch_adjacent_appointment_addresses
        return unless @schedule

        # Use preloaded appointments if available (set by therapist_query_helper)
        day_appointments = if @therapist.respond_to?(:preloaded_appointments)
          @therapist.preloaded_appointments
        else
        ...
        # Find previous and next appointments from the preloaded collection
