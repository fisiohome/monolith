...
export default function ToolbarTable({ table: _ }: { table: TableToolbarDataProps }) {
  ...
  const updateQueryParams = useCallback(
    debounce((value) => {
      const { fullUrl, queryParams } = populateQueryParams(pageURL, { ...value });

      router.get(fullUrl, { ...queryParams }, { preserveState: true, only: ['admins'] });
    }, 500),
    [pageURL]
  );
  const handleFilterBy = ({ event, type }: { event: ChangeEvent<HTMLInputElement>, type: 'email' }) => {
