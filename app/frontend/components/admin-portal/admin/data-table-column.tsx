import type { PageProps, TableRowDataProps } from "@/pages/AdminPortal/Admin/Index"
  ...
  return [
          ...
          <div className='flex items-start space-x-2'>
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent side='top'>
                  <span>{table.getIsAllRowsExpanded() ? 'Collapse all' : 'Expand all'}</span>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </div>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return (
          <>
            <Badge variant="secondary">{humanize(row.original.adminType).toUpperCase()}</Badge>
          </>
        )
      }
    },
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent side='bottom'>
                {isOnline ? (
                  <span>Current IP: <b>{currentIP}</b></span>
                ) : (
                  <div className='flex flex-col'>
                    <span>Last IP: <b>{lastIP}</b></span>
                    {lastSignIn && <span>Last Sign in: <b>{format(lastSignIn, 'PPpp')}</b></span>}
                  </div>
                )}
              </TooltipContent>
            </Tooltip>
