...
export type TableToolbarDataProps = TableTanstack<PageProps['admins']['data'][number]>
...
export default function Index({ admins }: PageProps) {
  ...
  const handleTabClick = (value: typeof tabList[number]['value']) => {
    let baseUrl = pageURL.split('?')[0];
    const { fullUrl, queryParams } = populateQueryParams(baseUrl, { 'filter_by_account_status': value });

    router.get(fullUrl, { ...queryParams }, { replace: true, preserveState: true, only: ['admins'] });
  }
  ...
  const handleDelete = (row: TableRowDataProps) => {
    ...
    console.log(`Successfully to deleted the Admin ${row.original.user.email}...`)
  }
    ...
    <>
      ...
      <article className="min-h-[100vh] flex-1 rounded-xl bg-muted/50 md:min-h-min p-6 space-y-4">
        <section className="flex items-center justify-between">
          ...
          {isSuperAdmin && (
            <Button asChild>
              <Link href={globalProps.adminPortal.router.adminPortal.adminManagement.new}>
                <Plus />
                Add Admin
              </Link>
            </Button>
            ...
            {tabList.map((tab) => (
              <Fragment key={tab.value}>
                <TabsContent value={tab.value}>
                  <DataTable
                    columns={columns({ isSuperAdmin, handleDelete, globalProps })}
                    data={admins.data}
                    toolbar={(table) =>
